...
function getUsersFromDatabase(array) {
  Array.from(array,function(elem, index){
...
function filterByCondition(arr, condition) {
  let newAr = arr.filter(condition);
  return newAr;
};
...
function copyAndSwapElements(arr, target, start, end) {
  if (0 <= target < arr.length && 0 <= start < arr.length && end < arr.length) {
...
function customIncludes(arr, element) {
  if (Array.isArray(arr)) {
    const checkEl = arr.includes(element);
